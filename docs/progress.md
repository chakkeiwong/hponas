...
> Campaign spend remains blocked until the protocol re-review completes, the pilot sizing report is delivered, and PI certification is signed. Tier 0 implementation (SearchSpace, Store schema, RLlib adapter, test harness) is engineering work and proceeds in parallel with protocol sign-off.
...
**Gate report:** [r1_gate_2026-08-31.md](../gate_reports/r1_gate_2026-08-31.md)
...
1. **Sobol state loss on resume (correctness bug).** `SobolSearcher` held QMC state in `self._sobol` with a no-op `observe()`, so a rebuilt searcher restarted the sequence: 16 trials covered only 8 distinct points, each evaluated twice. The example's own assertion tested `trial_id` uniqueness, which is trivially true and always passed. **Fixed:** added `state_dict()`/`load_state_dict()` to the `Searcher` protocol, implemented for Sobol via `fast_forward()`, persisted per-trial in the example, and strengthened the assertion to compare configs. Resumed sequence now equals the uninterrupted run point-for-point. 4 new contract tests.
2. **RayExecutor is a stub**, so the local-vs-Ray conformance check was local-vs-local. **Resolved as scope:** real Ray Tune is Tier 0 work (~4d) per the charter; recorded as a known limitation in SPIKE_REVIEW.md rather than an unmet criterion.

**Interface change carried into Tier 0:** every searcher (GP+qLogEI, TPE, TuRBO) must implement the state serialization contract. Counter-based fast-forward will not generalize to surrogate-carrying searchers — they need real state capture.
...
**Gate:** V01, V02, V03, V04-T0, V05, V14
**Program:** [BUILD_PROGRAM_v2.md](../BUILD_PROGRAM_v2.md)
**Approval:** [BUILD_PROGRAM_v2_APPROVAL.md](../BUILD_PROGRAM_v2_APPROVAL.md)

Scope and effort from approved v2: ~65 engineer-days over 6 weeks. Wraps Sobol/TPE/DEHB searchers, ASHA/Median schedulers, run store, and RLlib adapter. Campaign spend is blocked on validation protocol sign-off; implementation proceeds in parallel.
...
**Status:** DRAFT — 18 entries
**Register:** [validation/protocols.json](../validation/protocols.json)
**Checker:** `python3 tools/check_protocols.py`

Repairs findings F1-F3 and items 1-8 of RESPONSE_TO_CODEX_STATISTICAL_REVIEW.md. Awaiting: Codex re-review, pilot report, PI certification.

**Policy decisions:** 2 of 5 resolved.

- **D1** (PROPOSED — requires program sign-off): Confirmatory family scope: per-gate (chosen) vs global across all 15 entries vs hierarchical gatekeeping.
- **D2** (PROPOSED — requires program sign-off): Power target: 0.8 default with 0.9 for V04-T1 and V08 (chosen), vs 0.9 throughout.
- **D3** (PROPOSED — requires program sign-off): V08 hold semantics: time-limited opt-in with auto-demotion on expiry (chosen), vs hold blocks the beta gate.
...
| 2026-09-01 | Confirmatory family is the gate, not the entry | One primary per entry is not one comparison per decision; six uncorrected primaries carry a ~0.26 family-wise false-positive rate (D1) |
